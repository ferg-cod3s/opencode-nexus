...
impl ServerManager {
    ...
    ///
    /// Design Decision: Returns real session data from /session endpoint
    ...
    pub async fn get_active_sessions(&mut self) -> Result<Vec<OpenCodeSession>> {
        ...
        if let Some(client) = &self.api_client {
            match client.get::<Vec<OpenCodeSession>>("/session").await {
                Ok(sessions) => {
    ...
    ///
    /// Design Decision: Uses DELETE /session/{id} endpoint
    ...
    pub async fn disconnect_session(&mut self, session_id: &str) -> Result<bool> {
        ...
        if let Some(client) = &self.api_client {
            match client
                .delete::<bool>(&format!("/session/{}", session_id))
                .await
            {
                Ok(success) => {
    ...
    async fn check_server_connectivity(&self) -> Result<()> {
        if let Some(client) = &self.api_client {
            ...
            let endpoints = vec!["/session", "/app/stats"];
...
mod tests {
    ...
    fn test_port_availability_check() {
        ...
        let test_port = 54321;
        let port_available = test_manager.manager.is_port_available(test_port).unwrap();

        // The test should not crash and should return a boolean result
        assert!(port_available == true || port_available == false);

        // Test with a system port that should typically be unavailable
        let system_port_available = test_manager.manager.is_port_available(80).unwrap();

        // System ports (0-1023) should typically be unavailable on most systems
        // This test is environment-dependent, so we just ensure it doesn't crash
        assert!(system_port_available == true || system_port_available == false);
    }
    ...
    async fn test_tunnel_lifecycle_integration() {
        ...
        let status = test_manager.manager.get_tunnel_status();
        assert!(matches!(status, TunnelStatus::Stopped));
        ...
        let result = test_manager.manager.start_cloudflared_tunnel(&config).await;
        assert!(result.is_ok(), "Failed to start tunnel: {:?}", result.err());
        
        // Test 3: Verify tunnel status after starting
        let status = test_manager.manager.get_tunnel_status();
        assert!(matches!(status, TunnelStatus::Running));
        
        
        
        // Test 4: Verify tunnel config is stored
        let stored_config = test_manager.manager.get_tunnel_config();
        assert!(stored_config.is_some());
        assert_eq!(stored_config.as_ref().unwrap().enabled, true);
        
        // Test 5: Stop tunnel
        let result = test_manager.manager.stop_cloudflared_tunnel();
    ...
    async fn test_tunnel_config_persistence() {
        ...
        // Update config
        let result = test_manager.manager.update_tunnel_config(config.clone());
        assert!(result.is_ok());
        ...
        let stored_config_again = test_manager.manager.get_tunnel_config();
    ...
    async fn test_concurrent_tunnel_operations() {
        ...
        // Start first tunnel
        let result1 = test_manager.manager.start_cloudflared_tunnel(&config1).await;
        ...
        let status = test_manager.manager.get_tunnel_status();
        assert!(matches!(status, TunnelStatus::Running) || matches!(status, TunnelStatus::Stopped));
    }
    ...
    async fn test_tunnel_url_extraction() {
        ...
        let url = test_manager.manager.get_tunnel_url();
        assert!(url.is_some());
    ...
    async fn test_tunnel_process_cleanup() {
        ...
        test_manager.manager.start_cloudflared_tunnel(&config).await.unwrap();
        
        // Verify tunnel is running
        let status_before = test_manager.manager.get_tunnel_status();
        assert!(matches!(status_before, TunnelStatus::Running));
        
        // Stop tunnel
        test_manager.manager.stop_cloudflared_tunnel().unwrap();
        
        // Verify tunnel is stopped and resources are cleaned up
        let status_after = test_manager.manager.get_tunnel_status();
        assert!(matches!(status_after, TunnelStatus::Stopped));
        
        
        // Verify config is cleared
        let config_after = test_manager.manager.get_tunnel_config();
