...
1. The `ci_scripts/pre_xcode_build.sh` script is automatically executed before the Xcode build
2. This script runs:
...
**Important:** Xcode Cloud will fail if this pre-build script is not present or doesn't have execute permissions.

## Notes

- The `gen/apple` directory is generated by Tauri and should be committed for Xcode Cloud builds
- Build artifacts within `gen/apple` (such as the `build/` directory) remain git-ignored
...
- The pre-build script must have execute permissions: `chmod +x ci_scripts/pre_xcode_build.sh`
