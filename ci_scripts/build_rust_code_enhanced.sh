#!/bin/bash
# Enhanced CI-Compatible Rust Build Script for Tauri iOS
# 
...
#   ${CI_PRIMARY_REPOSITORY_PATH}/ci_scripts/build_rust_code_enhanced.sh
...
echo "Alternative path: $ALT_LIBAPP_PATH"
...
# Enhanced function to check if libapp.a is valid for arm64
validate_library() {
    local path="$1"
    local description="$2"
    ...
    if [ -f "$path" ]; then
        ...
        if file "$path" | grep -q -E "(ar archive|current ar archive)"; then
            ...
            if lipo -info "$path" 2>/dev/null | grep -q "arm64"; then
                echo "✅ Valid arm64 library found: $path ($description)"
                return 0
            ...
            echo "✅ Valid archive library found: $path ($description)"
            return 0
        else
            echo "⚠️ File exists but not a valid archive: $path ($(file "$path"))"
        fi
    else
        echo "📁 Library not found: $path"
    fi
    return 1
}

# First, check if libapp.a already exists (pre-built by CI or previous build)
if validate_library "$LIBAPP_PATH" "primary path"; then
    echo "🎯 Pre-built library found, skipping Rust compilation"
    exit 0
fi

if validate_library "$ALT_LIBAPP_PATH" "alternative path"; then
    echo "🔄 Copying library from alternative path to primary path..."
    mkdir -p "$OUTPUT_DIR"
...
if [ "$IS_CI" = "true" ]; then
    ...
    # Build the Rust library with correct name
    echo "🔨 Building: cargo build --target aarch64-apple-ios $CARGO_FLAGS --lib"
    echo "   Expected library name: libsrc_tauri_lib.a"
    
    if ! cargo build --target aarch64-apple-ios $CARGO_FLAGS --lib; then
    ...
    # Try the correct library name first (from Cargo.toml)
    LIB_NAME="libsrc_tauri_lib.a"
    FOUND_LIB="${RUST_TARGET_DIR}/${LIB_NAME}"
    
    if [ ! -f "$FOUND_LIB" ]; then
        echo "⚠️ Expected library not found, searching for alternatives..."
        # Try other possible library names
        for ALT_LIB_NAME in "libapp.a" "libsrc_tauri.a" "libopencode_nexus.a"; do
            ALT_FOUND_LIB="${RUST_TARGET_DIR}/${ALT_LIB_NAME}"
            if [ -f "$ALT_FOUND_LIB" ]; then
                FOUND_LIB="$ALT_FOUND_LIB"
                echo "   Found alternative: $ALT_LIB_NAME"
                break
            fi
        done
        
        # If still not found, search for any .a file
        if [ ! -f "$FOUND_LIB" ]; then
            FOUND_LIB=$(find "$RUST_TARGET_DIR" -maxdepth 1 -name "lib*.a" -type f | head -1)
            if [ -n "$FOUND_LIB" ]; then
                echo "   Found any library: $(basename "$FOUND_LIB")"
            fi
        fi
    fi
    ...
    if [ -n "$FOUND_LIB" ] && [ -f "$FOUND_LIB" ]; then
        ...
        echo "✅ Copied $(basename $FOUND_LIB) to:"
        echo "   Primary: $LIBAPP_PATH"
        echo "   Alternative: $ALT_LIBAPP_PATH"
        
        # Validate the copied library
        if validate_library "$LIBAPP_PATH" "copied library"; then
            echo "🎉 CI build completed successfully"
            exit 0
        else
            echo "❌ Copied library validation failed"
            exit 1
        fi
    else
...
echo "🚀 Executing Tauri xcode-script..."
