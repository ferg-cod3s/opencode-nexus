...
const mockApi = {
  ...
  send_chat_message: async (args: { session_id: string; content: string }): Promise<void> => {
    ...
    // Simulate streaming by emitting chunks, then a final message
    const fullResponse = [
      `Got it. You said: "${args.content}".`,
      ' Here is a mock, streamed response with a few points:',
      '\n\n- This is a simulated assistant reply.',
      '\n- Responses are chunked to mimic streaming.',
      '\n- Replace with real backend streaming when available.'
    ].join('');
...
export const invoke = async <T = any>(command: string, args?: Record<string, any>): Promise<T> => {
  ...
  if (isTauriEnvironment()) {
    try {
      // Use real Tauri API - in Tauri v2, invoke is in the core module
...
export const listen = async (event: string, handler: (event: any) => void): Promise<() => void> => {
  if (isTauriEnvironment()) {
    try {
      // Use real Tauri event API - in Tauri v2, listen is in the event module
